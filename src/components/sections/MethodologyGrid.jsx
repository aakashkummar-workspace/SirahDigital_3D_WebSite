    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
      {METHODOLOGY.map((m, i) => (
        <Reveal key={m.title} delay={i * 120} className="h-full">
          <SpotlightCard
            className={`h-full p-8 backdrop-blur-xl rounded-3xl border pointer-events-auto transition-all duration-300 hover:-translate-y-1.5 ${m.hover} bg-space-deep/70 border-ink/5`}
          >
