...
export default function ResultPanel({ result, accent }) {
  ...
  return (
    <div className="min-w-0">
      ...
      <div className="mt-8">
        <p className="text-[13px] text-white/55">Estimated annual savings</p>
        <AnimatedCounter
          value={result.annualSavings}
          format={fMoney}
          className="mt-2 block font-semibold tabular-nums leading-[0.95] tracking-tight text-white"
      ...
      <dl className="mt-10 divide-y divide-white/[0.06] border-y border-white/[0.06]">
        {secondary.map((m) => (
          <div key={m.label} className="flex items-baseline justify-between gap-4 py-3.5">
            <dt className="text-[13px] text-white/55">{m.label}</dt>
            <dd className="flex shrink-0 items-baseline gap-1.5">
              <AnimatedCounter
                value={m.value}
                format={m.format}
                className="text-[16px] font-semibold tabular-nums text-white"
              />
              {m.suffix && <span className="text-[12px] text-white/40">{m.suffix}</span>}
            </dd>
          </div>
        ))}
      </dl>
      ...
      <section aria-labelledby="roi-coverage" className="mt-11">
        ...
        />
        <h3 id="roi-coverage" className="mt-2 text-[13px] text-white/55">
          Automation coverage
        </h3>
        <Meter value={result.automationCoverage} accent={accent} />
        <p className="mt-3 max-w-[46ch] text-[12px] leading-relaxed text-white/40">
      ...
      <section aria-labelledby="roi-productivity" className="mt-9">
        <div className="flex items-baseline justify-between gap-4">
          <h3 id="roi-productivity" className="text-[13px] text-white/55">
            Productivity improvement
          </h3>
          <AnimatedCounter
            value={result.productivityGain}
            format={fPctSigned}
            className="shrink-0 text-[16px] font-semibold tabular-nums text-white"
          />
        </div>
      ...
      <section aria-labelledby="roi-hours" className="mt-11">
        <h3 id="roi-hours" className="text-[13px] text-white/55">
        ...
        <div className="mt-5 space-y-4">
          <div>
            ...
            <div className="mt-2.5 h-[2px] rounded-sm bg-white/25" aria-hidden="true" />
          </div>

          <div>
            <div className="flex items-baseline justify-between gap-4">
              <span className="text-[13px] text-white/70">With Sirah</span>
              <AnimatedCounter
                value={after}
                format={fHoursFull}
            ...
            <div className="mt-2.5 h-[2px] rounded-sm bg-white/10" aria-hidden="true">
              <div
